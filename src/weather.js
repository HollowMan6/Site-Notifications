...
const pushWeather = function (subsinfo, hjson_day, data = "") {
  ...
  const title =
    day0.cond_txt_d +
    "，" +
    hjson_day.HeWeather6[0].basic.location +
    "，" +
  ...
  options.icon = encodeURI(
    "https://github.com/zhuguohui/WeatherTextViewDemo/raw/master/weather/src/main/assets/weather/" +
      day0.cond_txt_d +
      ".png"
  );
  ...
  if (data) {
    ...
    if (hjson_air.HeWeather6[0].status == "ok") {
      options.body +=
        "空气质量：" +
        hjson_air.HeWeather6[0].air_now_city.aqi +
        " " +
  ...
  options.data =
    ...
    hjson_day.HeWeather6[0].basic.cid +
    ".html";
    ...
    https
      .get(
        "https://free-api.heweather.net/s6/weather/forecast?location=" +
          location +
          "&key=90a2d6b5f3864f9cb2b5d591201e867a&lang=zh-hans",
        (resp) => {
          let data = "";
              ...
              https
                .get(
                  "https://free-api.heweather.net/s6/air/now?location=" +
                    hjson_day.HeWeather6[0].basic.parent_city +
                    "&key=e8410d236ac043bbb9939bd536e2e646&lang=zh-hans",
                  (resp) => {
                    let data = "";
